...
function heroAnimation () {
    ...
    gl.shaderSource(vertexShader, [
        ...
        'void main (void) {',
            //'fragCol = vertCol;',
            'float lift = 0.0; //rand(vec2(vertPos.x, vertPos.z));',
    ...
    for (var x = startX; x < width; x += step * 2, xIndex++) {
        for (var z = height; z > endZ; z -= step * 2, zIndex--) {
            var noise1 = simplexMachine.noise3D(x - step, 0, z + step)
            var noise2 = simplexMachine.noise3D(x - step, 0, z - step)
            var noise3 = simplexMachine.noise3D(x + step, 0, z + step)
